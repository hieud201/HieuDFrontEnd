...
export default function SearchBookView({ params }: Props) {
  ...
  useEffect(() => {
    const fetchBook = async () => {
      ...
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch(`http://localhost:4000/retrieve/retrieveISBN?ISBN=${params.isbn13}`, {
          method: 'GET',
          headers: {
        ...
        if (data.books && data.books.length > 0) {
          setBook(data.books[0]); // Take the first book since ISBN should be unique
        } else {
